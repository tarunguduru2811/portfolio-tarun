...
const WorkTimelineItem: React.FC<{ item: WorkItem; isLast: boolean }> = ({ item, isLast }) => {
  ...
  return (
    <div className="relative flex flex-col sm:flex-row gap-4 sm:gap-8 mb-8 z-10">
      {/* Icon + Line */}
      <div className="flex flex-col items-center">
        <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center bg-gray-900 border-2 border-teal-500 rounded-full z-10">
      ...
      {/* Card */}
      <div
        className="flex-1 bg-gray-900 border border-gray-700 rounded-xl shadow-lg p-4 sm:p-6 cursor-pointer hover:shadow-teal-500/20 transition-all"
      ...
      >
        <div className="flex flex-col sm:flex-row items-start gap-4">
          ...
          <div className="flex-1">
            <h4 className="text-lg font-semibold font-heading text-center sm:text-left">{item.role}</h4>
            <p className="text-teal-400 text-sm font-body text-center sm:text-left">{item.company}</p>
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:gap-4 font-body text-gray-400 text-sm mt-1">
...
const EducationTimelineItem: React.FC<{ item: EducationItem; isLast: boolean }> = ({ item, isLast }) => (
  <div className="relative flex flex-col sm:flex-row gap-4 sm:gap-8 mb-8 z-10">
    ...
    <div className="flex-1 bg-gray-900 border border-gray-700 rounded-xl shadow-lg p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row items-start gap-4">
        {item.logo && (
          <div className="relative w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0">
        ...
        <div className="flex-1">
          <h4 className="text-lg font-semibold font-heading text-center sm:text-left">{item.degree}</h4>
          <p className="text-teal-400 text-sm font-body text-center sm:text-left">{item.institute}</p>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:gap-4 font-body text-gray-400 text-sm mt-1">
            <span className="flex items-center gap-1">
              <Calendar size={14} className="text-teal-400" /> {item.duration}
            </span>
          ...
          <p className="text-gray-300 mt-2 font-body text-center sm:text-left">GPA: {item.gpa}</p>
        </div>
      </div>
    </div>
  </div>
